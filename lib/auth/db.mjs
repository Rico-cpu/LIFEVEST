...
function assertNotProductionDatabase() {
  ...
  // A Neon branch carries the SAME database name as its parent — a `dev`
  // branch is still `neondb`. Checking the name alone would therefore refuse a
  // perfectly correct branch. When the branch is explicitly configured, that is
  // the isolation, so allow it.
  if (process.env.VEYRA_NEON_BRANCH) return;
  ...
  if (name === productionName) {
    throw new Error(
      ...
      'Point it at your development database (or run scripts/neon-branch.mjs to use a ' +
      'Neon branch), or set VEYRA_ALLOW_PRODUCTION_DB=1 if you genuinely mean to.'
    );
  }
}
