...
test('authorization levels cap what may be prepared, and never who may authorize', () => {
  ...
  // Preparing an automation-originated action is allowed from level 2: Assisted
  // mode stages scheduled actions for the user to approve. Running one WITHOUT
  // asking is gated separately by autopilot.mayExecuteAutonomously().
  assert.equal(gate(LEVEL.PREPARE, { ...smallInvest, origin: 'automation' }).allowed, true);
  assert.equal(gate(LEVEL.READ_ONLY, { ...smallInvest, origin: 'automation' }).allowed, false);
