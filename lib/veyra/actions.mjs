 ...
 * @property {'direct'|'standing_rule'} [basis] how the authority was given
 * @property {string} [policyConsentVersion] the autopilot consent it rests on
 *
 ...
 * @property {any} [result]
 * @property {boolean} [replayed] true when returned from the idempotency ledger
 */
...
/**
 * Authorize an action under a standing rule the account holder set in advance.
 *
 * This is how Autopilot acts without a prompt, and it does not weaken the
 * human-only invariant: the authorizing actor is still the person, and the
 * record says the authority came from a rule they activated, naming the exact
 * consent version it rests on.
 *
 * Two things a standing rule can never do:
 *   - satisfy REQUIRE_REAUTH. Step-up authentication is a fresh act by
 *     definition; consent given last month is not a passkey tap now.
 *   - authorize anything the policy engine blocked.
 *
 * @param {ActionRecord} rec
 * @param {Object} opts
 * @param {Actor} opts.actor                the person who activated the rule
 * @param {{enabled:boolean, consentVersion:string|null}} opts.policy
 * @param {number} [opts.now]
 * @returns {ActionRecord}
 */
export function authorizeByStandingRule(rec, { actor, policy, now }) {
  if (actor.kind !== 'user') {
    throw new Error('a standing rule must name the person who activated it');
  }
  if (!policy?.enabled) {
    throw new Error('no active standing rule to authorize under');
  }
  if (rec.state !== STATE.PENDING_APPROVAL) {
    throw new TransitionError(rec.state, STATE.APPROVED);
  }
  ...
  if (rec.decision.verdict === VERDICT.REQUIRE_REAUTH) {
    throw new Error('authorization denied: this action requires re-authentication, which a standing rule cannot supply');
  }

  const at = new Date(now ?? Date.now()).toISOString();
  const next = transition(rec, STATE.APPROVED, { kind: 'automation', id: 'autopilot' }, 'standing rule');
  next.authorization = {
    actor,
    at,
    twinVersion: rec.decision.twinVersion,
    ...
    decisionVerdict: rec.decision.verdict,
    basis: 'standing_rule',
    policyConsentVersion: policy.consentVersion,
  };
  return next;
}
...
export async function execute(rec, executor, by, ledger) {
  ...
  if (ledger.has(rec.idempotencyKey)) {
    ...
    // Flagged, because callers must be able to tell a replay from a first
    // execution. A replay moved no money, so anything that accrues against it —
    // spend counters, velocity limits, notifications — must not fire again.
    return {
      ...rec,
      state: STATE.EXECUTED,
      result: prior,
      replayed: true,
      history: [...rec.history, { at: new Date().toISOString(), from: rec.state, to: STATE.EXECUTED, by, note: 'idempotent replay' }],
    };
  }
